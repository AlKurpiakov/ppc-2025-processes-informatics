  ...
  void SetUp() override {
    ...
    if (IsSeqTest()) {
      ...
      if (block_size > 0) {
        ...
        MPI_Scatter(sendbuf, recvcount, recvtype,
                    temp_buffer.data(), recvcount, recvtype,
                    root, comm);
...
std::array<TestType, 4> CreateTestParams() {
  ...
  return {
    std::make_tuple(
      ...
      "test_int",
      int_expected_buf.data()
    ),
    
    std::make_tuple(
      std::make_tuple(
        float_data.data(),      // sendbuf
        2,                      // sendcount
        MPI_FLOAT,              // sendtype
...
const auto kPerfTestName = PikhotskiyRScatterRunFuncTestsProcesses::PrintFuncTestName<PikhotskiyRScatterRunFuncTestsProcesses>;
